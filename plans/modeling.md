...
We're rebuilding the data layer as a **src-first library** and building Strategy B end-to-end.
...
- **`prices.py`** — chunked monthly price/volume download, cached to parquet.
- **`fundamentals.py`** — historical **quarterly** financials (`yf.Ticker.quarterly_financials/_balance_sheet/_cashflow`); compute valuation/quality/growth ratios; align each to its **filing-availability date**. Slow path (~3,000 tickers ≈ 30–45 min, rate-limited) — fetch once, cache aggressively, support incremental refresh.
